%% Binary Genetic Algorithm Function Script
function out = binaryGeneticAlgorithm(problem, params)
	% Problem
	...
	for(i = 1:n_pop)
		...
		pop(i).position = randi([0, 1], 1, n_var);
	...
	for(i = 1:max_it)
		...
		for(j = 1:numb_child / 2)
			...
			% Perform Crossover
			[pop_c(j, 1).position, pop_c(j, 2).position] = randomCrossover(p1.position, p2.position);
		end
		...
		for(k = 1 : numb_child)
			% Perform Mutation
			pop_c(k).position = mutate(pop_c(k).position, Mu, false, 0);
